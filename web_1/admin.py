...
def save_errors(self, request, queryset):
  def mysqlQuer(id_num, project):
    conn = MySQLdb.connect('192.168.6.120', 'trusme', '6286280300', 'command', charset='utf8')
    cursor = conn.cursor()
...
#                  'classes':('collapse',),
#                  'fields':(
#'status','sn','sn_1','time','name','family','cpu','memory','disk_num','bios','bmc','raid','disk','mac','network',
#                  'fru','name1','mac_addr','boot_time')
#                 }],
#                 
#                ['message',{
#                 'fields':('message',),
...
#            return info
#        else:
#            return super(HostAdmin,self).get_readonly_fields(request, obj)
...
#            fields = (('message', 'sel'), ('smart_info'),)
    ...
    def get_account_state(self, obj):
        if 'OS off' in obj.stress_test:
            return u'<a href=http://192.168.1.57/technology/web/Host/detail/%s>\
                   <span style="color:red;font-weight:bold">%s</span></a>' % (obj.id, obj.stress_test)
    ...
    def get_img(self, obj):
        ...
        if os.path.exists('/data/ftp/breakin/breakin-img/%s-1.png'% sn):
            return u'<a href=http://192.168.1.57/file/%s-1.png>%s</a>'%(sn, name1)
        elif os.path.exists('/data/ftp/breakin/breakin-img/%s-1.png'% sn_1):
            return u'<a href=http://192.168.1.57/file/%s-1.png>%s</a>'%(sn_1, name1)
        else:
            return name1
    ...
    def get_dmp(self, obj):
        ...
        if os.path.exists('/data/ftp/breakin/breakin-img/%s.dmp'% sn):
            return u'<a href=http://192.168.1.57/file/%s.dmp>%s</a>'%(sn, name)
        elif os.path.exists('/data/ftp/breakin/breakin-img/%s.dmp'% sn_1):
            return u'<a href=http://192.168.1.57/file/%s.dmp>%s</a>'%(sn_1, name)
        else:
            return name
    ...
    def button(self, obj):
        if 'OS off' in obj.stress_test:
            return u'<form action="/check/" method="post">\
                         <button name="check" style="height:20px;width:50px; type="submit" value="run">run</button>\
                     </form>'
        else:
            return u'<form action="/check/" method="post">\
                         <button name="check" style="height:20px;width:50px; type="submit" value="stop">stop</button>\
                     </form>'
    button.short_description = u'test'
    button.allow_tags = True

    def get_log(self, obj):
        id = obj.id
        message = obj.message
        with open('/data/ftp/breakin/breakin-img/message%s.dmp'%id, 'w') as fd:
            for i in message.split('\n'):
                info = i + '\r\n'
                fd.write(info)
        return u'<a href=http://192.168.1.57/file/message%s.dmp>%s</a>'%(id ,id)
...
#            return super(HostAdmin,self).get_readonly_fields(request, obj)
    ...
    def get_account_state(self, obj):
        if 'OS off' in obj.stress_test:
            return u'<a href=http://192.168.1.57/technology/web/HostCheck/detail/%s> \
                   <span style="color:red;font-weight:bold">%s</span></a>' % (obj.id, obj.stress_test)
        elif obj.stress_test == 'running':
            return u'<a href=http://192.168.1.57/technology/web/HostCheck/detail/%s> \
                  <span style="color:green;font-weight:bold">%s %s</span></a>' % (obj.id, obj.stress_test, obj.boot_time)
...
class SmartAdmin(admin.ModelAdmin):
    ...
    def get_smart(self, obj):
        return u'<a href=http://192.168.1.57/smart/%s>%s</a>' % (obj.id, obj.sn)
